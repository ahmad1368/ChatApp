 ...
 * capacity-limited RSVP with a waitlist).
 */
export default function GroupEventsPage() {
  const [author] = useState(() => getOrCreateGuestIdentity());
  const [events, setEvents] = useState<GroupEvent[]>([]);
  const [title, setTitle] = useState("");
  const [type, setType] = useState<"webinar" | "game">("game");
  ...
  const create = async () => {
    ...
    const res = await fetch(`${API_URL}/api/group-events`, {
      ...
      body: JSON.stringify({ host: author, title, type, startsAt: new Date(startsAt).toISOString(), capacity }),
    });
  ...
  return (
    <main style={{ maxWidth: 480, margin: "48px auto", padding: 16, fontFamily: "sans-serif" }}>
      ...
      <div style={{ border: "1px solid var(--color-border)", borderRadius: 8, padding: 12, marginBottom: 16 }}>
        ...
        <select value={type} onChange={(e) => setType(e.target.value as "webinar" | "game")} style={{ width: "100%", marginBottom: 8 }}>
        ...
        />
        <button onClick={create} disabled={!title.trim() || !startsAt}>
          Create
        </button>
      </div>
