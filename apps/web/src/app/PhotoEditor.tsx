 ...
 */
export default function PhotoEditor({ file, onSave, onCancel }: { file: File; onSave: (blob: Blob) => void; onCancel: () => void }) {
  ...
  const draw = () => {
    ...
    const preset = FILTER_PRESETS.find((p) => p.id === filterPreset)?.css ?? "";
  ...
  useEffect(draw, [scale, offsetX, offsetY, brightness, contrast, filterPreset]);
