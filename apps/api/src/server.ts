...
export function createApp(onboardingStore: OnboardingStore, uploadStore: UploadStore) {
  const app = express();
  app.use(cors());
  ...
  // NOTE: as with #26-#33, :userId is trusted from the URL rather than a
...
export function createChatServer(onboardingStore: OnboardingStore, uploadStore: UploadStore) {
  const { app, messagesByRoom } = createApp(onboardingStore, uploadStore);
...
if (require.main === module) {
  const httpServer = createChatServer(new OnboardingStore(), new UploadStore());
