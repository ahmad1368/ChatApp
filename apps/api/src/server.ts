...
export function createApp(facebookAuthService: FacebookAuthService, userStore: UserStore, tokenService: TokenService) {
  const app = express();
  app.use(cors());
...
export function createChatServer(facebookAuthService: FacebookAuthService, userStore: UserStore, tokenService: TokenService) {
  const { app, messagesByRoom } = createApp(facebookAuthService, userStore, tokenService);
...
if (require.main === module) {
  const httpServer = createChatServer(new FacebookAuthService(), new UserStore(), new TokenService());
