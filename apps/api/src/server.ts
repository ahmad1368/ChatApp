...
export function createApp(onboardingStore: OnboardingStore) {
  ...
  // NOTE: as with #26/#27, :userId is trusted from the URL rather than a
  // verified access token — this repo has no merged auth session yet
  // (#21-#25). Reading/writing another user's onboarding progress has low
  // stakes compared to those endpoints (no credentials or biometrics
  // involved), but should still be gated behind real auth before shipping.
