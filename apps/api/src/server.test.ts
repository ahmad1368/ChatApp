...
test("GET /api/video-call-effects/:author defaults to noise suppression on, the rest off (#131, #317)", async () => {
  const { server, baseUrl } = listen();
  try {
    const res = await fetch(`${baseUrl}/api/video-call-effects/alice`);
    assert.deepEqual(await res.json(), { effects: { beautyFilter: false, backgroundBlur: false, noiseSuppression: true } });
  } finally {
    server.close();
  }
});

test("PUT /api/video-call-effects/:author sets all three flags, then GET returns them (#131, #317)", async () => {
  const { server, baseUrl } = listen();
  try {
    const putRes = await fetch(`${baseUrl}/api/video-call-effects/alice`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ beautyFilter: true, backgroundBlur: true, noiseSuppression: false }),
    });
    assert.equal(putRes.status, 200);
    assert.deepEqual(await putRes.json(), { effects: { beautyFilter: true, backgroundBlur: true, noiseSuppression: false } });

    const getRes = await fetch(`${baseUrl}/api/video-call-effects/alice`);
    assert.deepEqual(await getRes.json(), { effects: { beautyFilter: true, backgroundBlur: true, noiseSuppression: false } });
  } finally {
    server.close();
  }
});
...
test("PUT /api/video-call-effects/:author rejects a missing author", async () => {
  ...
  try {
    const res = await fetch(`${baseUrl}/api/video-call-effects/${encodeURIComponent(" ")}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ beautyFilter: true, backgroundBlur: true, noiseSuppression: true }),
    });
    assert.equal(res.status, 400);
  } finally {
    server.close();
...
test("POST /api/spotify/connect rejects a missing author", async () => {
  const { server, baseUrl } = listenWithSpotify(
    new SpotifyService("client-id", "client-secret", async () => ({
      topTracks: ["Song A"],
      topArtists: [],
      moodValence: null,
      moodEnergy: null,
...
test("POST /api/spotify/connect connects and stores top tracks/artists, then GET/DELETE reflect it", async () => {
  const { server, baseUrl } = listenWithSpotify(
    new SpotifyService("client-id", "client-secret", async () => ({
      topTracks: ["Song A — Artist A"],
      topArtists: ["Artist A"],
      moodValence: 0.6,
      moodEnergy: 0.7,
    }))
...
test("PUT /api/spotify-info/:author updates the hide flag", async () => {
  ...
  try {
    ...
    assert.equal(res.status, 200);
    assert.deepEqual(await res.json(), {
      spotifyInfo: { connected: false, topTracks: [], topArtists: [], moodValence: null, moodEnergy: null, hideSpotify: true },
    });
  } finally {
    server.close();
  }
...
test("GET /api/artist-matches/:author returns candidates sharing at least one top artist (#326)", async () => {
  const { server, baseUrl, spotifyInfoStore } = listen();
  try {
    await fetch(`${baseUrl}/api/discovery/join`, {
      method: "POST",
    ...
    spotifyInfoStore.connect("alice", [], null, null, ["Artist A", "Artist B"]);
    spotifyInfoStore.connect("bob", [], null, null, ["Artist A", "Artist C"]);
    spotifyInfoStore.connect("carol", [], null, null, ["Artist D"]);

    const res = await fetch(`${baseUrl}/api/artist-matches/alice`);
    const body = await res.json();
    assert.deepEqual(body.candidates, [{ author: "bob", sharedArtists: ["Artist A"], compatibility: 50 }]);
  } finally {
    server.close();
  }
});

test("GET /api/artist-matches/:author returns nothing when the author hasn't connected Spotify", async () => {
  const { server, baseUrl, spotifyInfoStore } = listen();
  try {
    await fetch(`${baseUrl}/api/discovery/join`, {
      method: "POST",
    ...
    spotifyInfoStore.connect("bob", [], null, null, ["Artist A"]);
...
test("GET /api/artist-matches/:author excludes a candidate hiding their Spotify info", async () => {
  const { server, baseUrl, spotifyInfoStore } = listen();
  try {
    await fetch(`${baseUrl}/api/discovery/join`, {
      method: "POST",
    ...
    spotifyInfoStore.connect("alice", [], null, null, ["Artist A"]);
    spotifyInfoStore.connect("bob", [], null, null, ["Artist A"]);
    await fetch(`${baseUrl}/api/spotify-info/bob`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
