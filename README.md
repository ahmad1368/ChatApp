...
- #46 Safety Center and guide for safe meetups — `/safety` page with static safe-meetup tips plus "Share your date": `SafetyPlanStore` records who/where/when and issues a share code (`POST /api/safety/plans`); a trusted contact opens `/safety/shared/:code` (`GET /api/safety/plans/shared/:shareCode`) to view the plan read-only, no login required, matching the Bumble/Tinder "share my date" pattern (near-duplicate of #47 — see CLAUDE.md's reconciliation note, resolved when #47 merges)
