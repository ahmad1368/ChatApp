...
## Known backlog overlap: #75/#257 and #313

Issue #313 ("Ability to define preferences regarding pets at home") is an exact duplicate of already-implemented functionality in the auto-generated backlog — same feature, no differentiating angle. #75's `PetsInfoStore` (`petsInfo.ts`) already has a fixed catalog that encodes real pet *preferences*, not just inventory: alongside `dog`/`cat`/etc., it includes `noPets`, `petFree`, `wantAPet`, and `allergicToPets` — exactly the "preferences regarding pets at home" #313 describes. #257 already extended `DiscoveryFiltersStore` (`discoveryFilters.ts`) with a `requiredPets` filter reading that same catalog against `/api/swipe-candidates/:author`, edited via `DiscoveryFiltersEditor` on `/settings/profile`. No separate implementation was made for #313 — skip it in `/next-issue` picks (see that file's step 1) and close it as a duplicate of #75/#257 once confirmed.

## Known backlog overlap: #228 and #282/#320

Issue #282 ("Ability to create a group to organize events") and issue #320 ("Ability to create singles study or work groups") are both exact duplicates of already-implemented #228 ("Ability to form interest groups (e.g., a hiking group)") in the auto-generated backlog — same feature (a persistent, joinable group whose members organize real-world/online activities), same reference app (Match.com), no differentiating angle. #228 already shipped exactly this: `InterestGroupStore` (`apps/api/src/interestGroups.ts`) lets any author create a named group with a free-text name/description (nothing restricts it to a hobby topic — a "Study Group" or "Job Networking Group" is just as valid a name as "Hiking Group"), others join/leave it, and members-only organize a capacity-limited, waitlisted `GroupActivity` scoped to that group. #320's "study or work" framing doesn't add anything #228's free-text naming doesn't already cover. No separate implementation was made for #282 or #320 — skip them in `/next-issue` picks (see that file's step 1) and close each as a duplicate of #228 once confirmed.
...
Issue #315 ("Show mental health status or psychological tendencies based on tests") was deliberately scoped down from its literal title. This app does not, and should not, display a "mental health status" on a profile — an unverified clinical/health disclosure shown publicly on a swipeable dating profile is a real discrimination/stigmatization risk with no legitimate offsetting benefit (no real clinician or verification stands behind it), unlike anything real dating apps (including eHarmony, the issue's own reference app) actually ship. The legitimate half of the request — a real "test" someone takes in-app, as opposed to a self-picked label like #76's MBTI/Enneagram — was implemented instead: `AttachmentStyleStore` (`apps/api/src/attachmentStyleQuiz.ts`) scores a real, established two-dimension (anxiety/avoidance) relationship-attachment-style framework from an 8-statement Likert-scale quiz, explicitly disclosed everywhere (code comments, the `/attachment-style-quiz` page copy) as a self-reflection tool, never a diagnosis or health status. If a future issue explicitly asks for clinical/health-status display again, apply the same reasoning rather than building it.
